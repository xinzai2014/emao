  ...
  methods: {
    ...
    initCityScroll() {
      ...
      this.cityScroll.on("scrollEnd", length => {
        let itemLength =
          _this.$refs.cityWrapper.children[0].children[0].offsetHeight;
...
.cityWrapper .cityInner li:not(:last-child) {
  border-bottom: 1px solid #999;
}
.cityWrapper .shadow {
  position: absolute;
...
.cityWrapper .shadow-top {
  top: 0;
}
.cityWrapper .shadow-bottom {
  bottom: 0;
}
.cityPopup .ttl {
  text-align: center;
